...
(define (filter p lst)
  ;;(writeln "Nova chamada de funcao")
  (define (filter-acc p lst newlst)
    ;;(write lst)
    ;;(write newlst)
    ;;(writeln "")
    (cond [(empty? lst) newlst]
          [(p (first lst)) (filter-acc p (rest lst) (append newlst (list(first lst))))]
          [else (filter-acc p (rest lst)  newlst)]))
  (filter-acc p lst '())
  )
...
(define (filtra-par-lg lst)
  (filter even? (filter integer? lst)))  ;; lst pode conter valores de qualquer tipo
...
(define (tamanho-lista-strings lstr)
  (foldr + 0 (map string-length lstr)))
...
;;
(define (todos? p lst)
  (equal? (length lst) (length (filter p lst))))
...
(define (partition p lst)
  (list (filter p lst) (filter (lambda (x) (not (p x))) lst)))
...
(define (divide-lista x lst)
  (map (lambda (y) (/ x y)) lst))

(define (filtra-menor lst)
  ;;(writeln "Chamada de função")
  (define (filtra-menor-acc lst acc)
    ;;(write lst)
    ;;(write acc)
    ;;(writeln "")
    (cond [(empty? lst) acc]
          [else (filtra-menor-acc
                 (filter
                       (lambda (x) (< x (first lst)))
                       lst)
                 (first lst))])
    )
  (if (empty? lst)
      '()
      (filtra-menor-acc lst (first lst)))
)


(define (menor-quociente x lst)
  (cond [(or (empty? lst) (equal? x 0))  '()]
        [else (filtra-menor (divide-lista x lst))]
        )
  )

(define-test-suite test-menor-quociente
  (test-equal? "Lista vazia"      (menor-quociente 12 '())               '())
  (test-equal? "Divisão por zero" (menor-quociente 0 (list 1 2 3 4 5))   '())
  (test-equal? "Exemplo professor"(menor-quociente 12 (list 2 3 4))      3)
  (test-equal? "Divisor 50"    (menor-quociente 50 (list 10 25 50))  1)
  (test-equal? "Fração"        (menor-quociente 1 (list 2 3 4))       1/4)
  )
