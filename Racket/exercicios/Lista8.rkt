...
;; --- EXERCISE VIII -------------------------------------------------------------------
;; Características imperativas em Racket
;;
...
(define (msg-n str n)
  (define (aux-tail-loop str n reverse-n)
    (if (= n 1)
      (displayln (~a str reverse-n))
      (begin
        (displayln (~a str reverse-n))
        (aux-tail-loop str (- n 1) (+ reverse-n 1))
      )
    )
  )

  (aux-tail-loop str n 1)
)
...
(define (multiplica-vetor-fold vec)
  (for/fold ([mult 1]) ([x vec])      
    (* mult x)
  )
)   
;; b) o menor valor entre os números de um vetor
(define (menor-valor-vetor-fold vec)
  (for/fold ([min (vector-ref vec 0)]) ([x vec])      
    (cond 
      [(< min x) (set! min x)]
    )
  )
...
(define (concatena-vetor-fold vec)
  (for/fold ([str ""]) ([x vec])      
    (string-append x str)
  )
)
;; d) a maior string em um vetor de strings
(define (maior-string-vetor-fold vec)
  (for/fold ([maior (vector-ref vec 0)]) ([x vec])      
    (cond
      [(> (string-length x) (string-length maior)) (set! maior x)]
    )
  )
) 
