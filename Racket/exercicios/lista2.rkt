...
(define (mult m n)
  ;; Uma multiplicação nada mais é do que a soma constante de m, n vezes
  (if (= n 0)
      0
      (+ m (mult m (+ n -1)))
      ))
...
;; usando apenas as funções add1(similar a ++) e sub1(similar a --). Pode ser assumido que
...
(define (max-lista l)
  (if (equal? l '())
      0
      (if (> (first l) (first(retorna-lista (max-lista (rest l)))))
          (first l)
